...
comPort = 'COM32'  # default com port
comPortBaud = 115200
...
class App:
    ...
    def send_to_serial(self):
        if self.ser.isOpen():
            self.ser.write(bytes(self.entryStr.get(), 'utf-8'))
                    ...
                    if self.image_current_row >= self.num_pixels:
                        ...
                        if len(words) >= 30:
                            ...
                            for v in words:
                                ...
                                self.display_pixel(self.num_pixels - 1 - self.image_current_row,
                                                   self.num_pixels - 1 - x, colour)
                                x += 1
                            self.image_current_row += 1
                        else:
                            print(
                                "line " + str(self.image_current_row) + "incomplete (" + str(len(words)) + " of " + str(
                                    self.num_pixels) + "), ignoring")
                            print("bad line: " + str(line))

                if str(line).find("image data") >= 0:
                    self.line_processed = True
    ...
    def display_pixel(self, x, y, colour):
        if 0 <= x < self.num_pixels and 0 <= y < self.num_pixels:
            ...
            fillColour = "#%02x%02x%02x" % (int(colour), int(colour), int(colour))
            # draw a new pixel and add to pixel_array
            self.new_pixel = self.canvas.create_rectangle(x * self.grid_size, y * self.grid_size,
